...
def iface_finder():
    try:
        ...
        for line in ipr.communicate()[0].decode().splitlines():
            if 'default' in line:
                l = line.split()
                iface = l[4]
                return iface
    ...
    if MsgType == b"\x0a" and EncType == b"\x17" and MessageType == b"\x02":
        if Data[49:53] == b"\xa2\x36\x04\x34" or Data[49:53] == b"\xa2\x35\x04\x33":
            ...
            if HashLen == 54:
                ...
                BuildHash = "$krb5pa$23$" + Name.decode() + "$" + Domain.decode() + "$dummy$" + Hash.hex()
...
def parse_ftp(full_load, dst_ip_port):
    ...
    ftp_user = re.match(ftp_user_re, full_load.decode(errors='ignore'))
    ftp_pass = re.match(ftp_pw_re, full_load.decode(errors='ignore'))
...
def mail_logins(full_load, src_ip_port, dst_ip_port, ack, seq):
    ...
    if src_ip_port in mail_auths:
        if seq in mail_auths[src_ip_port][-1]:
            ...
            try:
                ...
                msg = 'Mail authentication: %s' % decoded.decode()
                printer(src_ip_port, dst_ip_port, msg)
            except TypeError:
                pass
    ...
    # seq always = last ack of tcp stream
        ...
        if seq in mail_auths[dst_ip_port][-1]:
            ...
            if full_load.startswith(b'235') and b'auth' in full_load.lower():
            ...
            elif b' fail' in full_load.lower():
                # Reversed the dst and src
                printer(dst_ip_port, src_ip_port, a_f)
                found = True
                try:
    ...
    # This handles most POP/IMAP/SMTP logins but there's at least one edge case
        ...
        mail_auth_search = re.match(mail_auth_re, full_load.decode(errors='ignore'), re.IGNORECASE)
        if mail_auth_search != None:
            auth_msg = full_load.decode(errors='ignore')
        ...
        elif re.match(mail_auth_re1, full_load.decode(errors='ignore'), re.IGNORECASE) != None:
...
def irc_logins(full_load, pkt):
    ...
    Find IRC logins
    '''
    user_search = re.match(irc_user_re, full_load.decode(errors='ignore'))
    pass_search = re.match(irc_pw_re, full_load.decode(errors='ignore'))
    pass_search2 = re.search(irc_pw_re2, full_load.decode(errors='ignore').lower())
    if user_search:
...
def other_parser(src_ip_port, dst_ip_port, full_load, ack, seq, pkt, verbose):
    ...
    http_line, header_lines, body = parse_http_load(full_load.decode(errors='ignore'), http_methods)
    ...
    NTLMSSP2 = re.search(NTLMSSP2_re, full_load.decode(errors='ignore'), re.DOTALL)
    NTLMSSP3 = re.search(NTLMSSP3_re, full_load.decode(errors='ignore'), re.DOTALL)
    if NTLMSSP2:
...
def parse_netntlm_resp_msg(headers, resp_header, seq):
    ...
    if header_val3[0] == 'NTLM' or header_val3[0] == 'Negotiate':
        try:
            msg3 = base64.b64decode(header_val3[1])
...
def parse_ntlm_resp(msg3, seq):
    ...
    if len(msg3) > 43:
        ...
        lmhash = binascii.b2a_hex(msg3[lmoff:lmoff + lmlen]).decode()
        nthash = binascii.b2a_hex(msg3[ntoff:ntoff + ntlen]).decode()
        domain = msg3[domoff:domoff + domlen].replace(b"\0", b"").decode()
        user = msg3[useroff:useroff + userlen].replace(b"\0", b"").decode()
